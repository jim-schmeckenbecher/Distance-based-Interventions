...
Calculations were done in R [@rcoreteam2020] using the package metafor for the three level model [@viechtbauer2010c] and the package clubSandwich [@pustejovsky2021a] for the RVE correction. All Data needed for full automatic reproducibility are publicly available on Github.[^1]
...
![Note: Dotted Lines represent Study Cluster Dependencies](01_Plots_Tables/Forest_Plot.png "Forest Plot, ordered according to Moderator: Thhoughts vs. Behaviours")
...
Comparing the combined control-group vs. TAU, retuned that DBI giving alongside TAU were non-significantly less effected (*SMD=* `r op(Overall.CRVECI$beta[1])` CI95%[`r op(Overall.CRVECI$CI_L[1])`; `r op(Overall.CRVECI$CI_U[1])`]), than studies giving DBI using a waitlist or attention placebo control group. Heterogeneity remained significant at *Q* (df = `r OverallC$ddf[[1]]`) = `r op(OverallC$QE)`, *p =* `r op(OverallC$QEp)`.

Covariance was investigated by simultaneous visualization of control-group and outcome category. Suicidal behaviours and suicidal thoughts were unevenly distributed between different control-group types (see figure 2). An exploratory analysis including both moderators was implemented. When including both moderators, the difference between control groups became non-significant, with *SMD=* `r op(Overall.TRVECITT$beta[3])` CI95%[`r op(Overall.TRVECITT$CI_L[3])`; `r op(Overall.TRVECITT$CI_U[3])`], but suicidal acts and suicidal thoughts remained a significant moderator, with *SMD =* `r op(Overall.TRVECITT$beta[2])` CI95%[`r op(Overall.TRVECITT$CI_L[2])`; `r op(Overall.TRVECITT$CI_U[2])`] in favour of suicidal thoughts; heterogeneity was not significant at *Q* (df = `r Overall.TT$ddf[[1]]`) = `r op(Overall.TT$QE)`, *p =* `r op(Overall.TT$QEp)`.

![Funnel Plot, including all outcomes according to moderator type](01_Plots/Funnel.plot.png)
...
Both, inclusion of NSSI outcomes (*o* = `r sum(Total$NSSI)`) and exclusion of suicide outcomes (*o* = `r sum(Total$Suicide)`) had notable, respective negligible impacts on the subgroup of behaviour outcomes. Inclusion of NSSI increased the effectiveness to SMD= `r op(Overall.TRVE2$beta[1])` from SMD= `r op(Overall.TRVE$beta[1])`. and exclusion of suicide studies decreased effectiveness from SMD = `r op(Overall.TRVE$beta[1])` to SMD= `r op(Overall.TSRVE$beta[1])`. Note only @franklin2016 reported NSSI measures.
...
![Quality Assessment of all independent studies](01_Plots_Tables/RoB-II.pdf)
...
We contextualise our findings using meta-analyses of psychotherapeutic face-to-face interventions for suicidal ideation and/or behaviour. While it can be argued that comparing multiple meta-analyses is biased, as each has different inclusion criteria, designs and underlying assumptions, it nevertheless provides a rough picture of what can be expected from an intervention in this area. To maximise comparability, we have only included meta-analyses of RCT studies from the last ten years that used TAU, Wairlist or attention-placebo control groups.
...
We showed that DBI significantly reduced suicidal behaviours(*SMD*= `r op(Overall.TRVECI$beta[1])` CI95%[`r op(Overall.TRVECI$CI_L[1])`; `r op(Overall.TRVECI$CI_U[1])`]). When comparing the results of our meta-analysis on DBI with the results of nine meta-analyses on face-to-face interventions, we found three meta-analyses with some significantly stronger results, while six meta-analyses had only non-significantly different results.

Two of the three meta-analyses had adolescent samples [@kothgassner2020a; @bahji2021a] and showed significantly greater reductions in self-injury with Dialectical Behavioural Therapy (DBT) at post-treatment [@kothgassner2020a; @bahji2021a], compared with our results for DBI; but no longer at three-month follow-up [@bahji2021a]. In addition, one of the two meta-analyses also included eclectic therapy (ET) [@bahji2021a], with ET leading to a significantly greater reduction in self-harm post-intervention than our results for DBI; however, ET led to a significant increase in self-harm at three months follow-up.

The third meta-analysis that reported stronger results compared to our results for DBI included only psychoanalytic approaches [@briggs2019]. Psychoanalytic approaches reduced self-harm significantly more than our results for DBI at 6 months, but no longer at 12 months. All three meta-analyses predominantly used TAU as control groups.

This comparison shows that face-to-face interventions are likely to be more effective than DBI, but because the research is still underpowered, therapeutic approaches cannot be distinguished with statistical certainty.

In turn, confidence intervals are important in suicide research, as they can tell us the clinical potential of an intervention. For example, the effect size of @briggs2019 against suicide attempt episodes at 12 months follow-up is -reported as Number Needed to Treat- (NNT)- = 7.4 CI95% [Inf; 3.6]. This means that while this intervention may be ineffective (infinitely many patients need treatment), it can potentially help one in four patients. In contrast, the distance-based intervention (DBI) is reported as NNT = 29.5 CI95% [59; 22.1]. This means that reduction of suicidal behaviours has a 95% certainty, but at best one in twenty-two patients can be helped.

Therefore, DBIs, with their small and consistent reduction in suicidal behaviour, are useful. Both for those who are unable to receive face-to-face treatment and for those who are unwilling to seek face-to-face treatment. However, more effective methods such as DBT or psychoanalysis must be used in standard treatment.

### Effectiveness of Distance Based Interventions for suicidal thoughts

Suicidal thoughts are reduced by Distance Based Interventions (`r op(Overall.T2RVECI$beta[1])` CI95%[`r op(Overall.T2RVECI$CI_L[1])`; `r op(Overall.T2RVECI$CI_U[1])`]). However, face-to-face interventions are more effective against suicidal ideation [@hetrick2016], than the results we found for DBI. And although most meta-analyses with therapeutic approach specific inclusion criteria returned more comparable results, this is mostly a case of inferior power.

But, due to the large number of individuals suffering from suicidal ideation [@borges2010] and the superior scalability of DBI [@batterham2015], we see a role for DBI in the treatment of suicidal ideation, as DBI are far from ineffective. 
